...
const Analiz = () => {
  ...
  dataChart = [
    {
      title: "bajarilgan",
      value: chartRes.finishedPercent,
      color: "#eeeeee",
      price: chartRes.finishedSum,
    },
    {
      title: "jarayonda",
      value: chartRes.pendingPercent,
      color: "#eeeeee",
      price: chartRes.pendingSum,
    },
    {
      title: "qarzdor",
      value: chartRes.debtPercent,
      color: "#eeeeee",
      price: chartRes.debtSum,
    },
  ];

  const { data: resData } = useAxios({
    url: `/order/getOrderByStatus/${url}`,
    method: "get",
  });
  const { data: newData } = useAxios({
    url: `/order/getOrderByStatus/1 `,
    method: "get",
  });
  console.log(resData);
  React.useEffect(() => {
            ...
            orderRes.map((item) => {
              ...
              return (
                <About
                  ...
                  loading={loading}
                  setChartRes={setChartRes}
                  setOrdderRes={setOrdderRes}
                  url={url}
                />
              );
            })
