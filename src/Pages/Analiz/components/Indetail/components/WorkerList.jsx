...
const WorkerList = () => {
  ...
  function handlerWorker(id, workerT) {
    ...
    if (
      workerT == "CONSTRUCTOR" &&
      !dataWorkers.constructorList.some((i) => i === id)
    ) {
      dataWorkers.constructorList.push(id);
    }
    if (
      workerT == "INSTALLER" &&
      !dataWorkers.installerList.some((i) => i === id)
    ) {
    ...
    if (
      workerT == "GLAZIER" &&
      !dataWorkers.glaizerList.some((i) => i === id)
    ) {
  ...
  async function handleRemove(userId, workerT) {
    ...
    switch (workerT) {
      case "ANGEL":
        dataWorkers.angelList = dataWorkers.angelList.filter(
          (i) => i !== userId
        );
        break;
      case "CONSTRUCTOR":
        dataWorkers.constructorList = dataWorkers.constructorList.filter(
          (i) => i !== userId
        );
        break;
      case "DRIVER":
        dataWorkers.driverList = dataWorkers.driverList.filter(
          (i) => i !== userId
        );
        break;
      case "GLAZIER":
        dataWorkers.glaizerList = dataWorkers.glaizerList.filter(
          (i) => i !== userId
        );
        break;
      case "INSTALLER":
        dataWorkers.installerList = dataWorkers.installerList.filter(
          (i) => i !== userId
        );
        break;
    }
                ...
                return (
                  <div
                    ...
                    className={`flex justify-between text-black font-bold text-base cursor-pointer pb-1 px-3 ${
                      toggleActive(index) ? "bg-neutral-100" : "bg-stone-100"
                    } hover:bg-gray-200`}
                  >
